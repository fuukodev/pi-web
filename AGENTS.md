...
Global defaults are edited only from Settings → Models → Defaults (`components/ModelsConfig.tsx` → `app/api/settings/route.ts` → `lib/pi-settings.ts`). That route is the single auditable write path: it validates the model against cwd-bound services, reports the *effective* model after scope+auth resolution, writes through the SDK `SettingsManager` (shared lock + field merge with the TUI/CLI), and invalidates the models cache. See `docs/adr/0006-chat-model-selection-vs-global-defaults.md`.
