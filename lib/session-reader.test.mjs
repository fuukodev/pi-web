...
test("session listing classifies consultation relations without opening the full child", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "pi-web-consultation-prefix-"));
  const filePath = join(dir, "child.jsonl");
  const parentPath = join(dir, "parent.jsonl");
  const childId = "consultation-child";
  writeFileSync(filePath, [
    JSON.stringify({ type: "session", version: 3, id: childId, timestamp: "2026-01-01T00:00:00.000Z", cwd: dir, parentSession: parentPath }),
    JSON.stringify({ type: "model_change", id: "model", parentId: null, timestamp: "2026-01-01T00:00:00.000Z", provider: "test", modelId: "test-model" }),
    JSON.stringify({ type: "session_info", id: "name", parentId: "model", timestamp: "2026-01-01T00:00:00.000Z", name: "question" }),
    JSON.stringify({
      type: "custom", customType: "pi-web:consultation", id: "meta", parentId: "name",
      timestamp: "2026-01-01T00:00:00.000Z",
      data: {
        version: 1, parentSessionId: "parent-id", parentSessionPath: parentPath,
        contextMode: "selection", source: { kind: "assistant_text", entryId: "a1", blockIndex: 0, excerpt: "answer" },
        promptVersion: 1, createdAt: "2026-01-01T00:00:00.000Z",
      },
    }),
  ].join("\n"));
  ...
  const originalOpen = SessionManager.open;
  listSessions = async () => [{
    path: filePath, id: childId, cwd: dir,
    created: new Date("2026-01-01T00:00:00.000Z"), modified: new Date("2026-01-01T00:00:01.000Z"),
    messageCount: 1, firstMessage: "question", allMessagesText: "", parentSessionPath: parentPath,
  }];
  SessionManager.open = () => { throw new Error("full session open is not allowed while listing"); };
  resetSessionListState();
  t.after(() => {
    listSessions = originalListSessions;
    SessionManager.open = originalOpen;
