...
test("consultation children nest with subagents instead of becoming roots", () => {
  const roots = buildSessionTree([
    session("parent", "2026-01-01T00:00:00.000Z"),
    session("consultation", "2026-01-02T00:00:00.000Z", {
      kind: "consultation",
      parentSessionId: "parent",
      contextMode: "selection",
      source: { kind: "assistant_text", excerpt: "answer" },
      promptVersion: 1,
    }),
  ]);

  assert.deepEqual(roots.map((node) => node.session.id), ["parent"]);
  assert.deepEqual(roots[0].children.map((node) => node.session.id), ["consultation"]);
});
