...
test("restores session when cwd is absent", async () => {
  ...
  assert.deepEqual(
    ...
    { requestedCwd: null, sessionId: "saved-session", subsessionId: null },
  );
});

test("restores a consultation child separately from its parent session", async () => {
  const { getInitialNavigation } = await loadSubject();

  assert.deepEqual(
    getInitialNavigation(new URLSearchParams({ session: "parent", subsession: "child" })),
    { requestedCwd: null, sessionId: "parent", subsessionId: "child" },
  );
});

test("ignores an orphan consultation-child parameter", async () => {
  const { getInitialNavigation } = await loadSubject();

  assert.deepEqual(
    getInitialNavigation(new URLSearchParams({ subsession: "child" })),
    { requestedCwd: null, sessionId: null, subsessionId: null },
  );
});
...
test("preserves a URL-encoded Windows path", async () => {
  ...
  assert.deepEqual(
    ...
    { requestedCwd: "C:\\Projects\\pi-web", sessionId: null, subsessionId: null },
  );
});
