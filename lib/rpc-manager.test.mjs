...
test("built-in subagents persist their selected resource policy", async () => {
  ...
  assert.match(startupSource, /else if \(subagentResources\) \{\s*resourceLoaderOptions/);
  ...
  assert.match(source, /suppressCompletionNotifications: consultation \|\| Boolean\(subagentResources\)/);
...
test("normal sessions restore persisted tool selections before loading resources", async () => {
  ...
  assert.match(startupSource, /readConsultationMetadata\(sessionManager\.getEntries\(\)/);
  assert.match(startupSource, /const consultation = options\.consultation === true \|\|/);
  assert.match(startupSource, /readSessionToolSelection\(sessionManager\.getEntries\(\)/);
  assert.match(startupSource, /const selectedToolNames = consultation[\s\S]*?subagentResources\?\.tools \?\? persistedToolNames \?\? requestedToolNames/);
  ...
  assert.match(startupSource, /resourceLoaderOptions = CHAT_ONLY_RESOURCE_LOADER_OPTIONS/);
  assert.match(startupSource, /let trustReloadOptions[\s\S]*?if \(subagentResources\)[\s\S]*?projectTrustReloadOptions\(sessionCwd, agentDir\)/);
