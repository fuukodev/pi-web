...
test("built-in subagents persist their selected resource policy", async () => {
  ...
  assert.match(startupSource, /resourceLoaderOptions: subagentResources\s*\?\s*withPiWebBuiltInExtensions\(/);
...
test("normal sessions restore persisted tool selections before loading resources", async () => {
  ...
  assert.match(startupSource, /chatOnly\s*\? withPiWebBuiltInExtensions\(CHAT_ONLY_RESOURCE_LOADER_OPTIONS\)/);
