...
test("RPC session startup preloads extension-registered providers before restoring models", async () => {
  ...
  assert.match(startupSource, /withPiWebLlamaProvider\(/);
  assert.doesNotMatch(startupSource, /: withPiWebBuiltInExtensions\(/);
  assert.match(startupSource, /refreshPiWebLlamaModels\(/);
...
test("built-in subagents persist their selected resource policy", async () => {
  ...
  assert.match(startupSource, /resourceLoaderOptions: subagentResources\s*\?\s*withPiWebLlamaExtensions\(/);
  assert.match(startupSource, /includeCommands: subagentResources\.loadExtensions/);
...
test("RPC session startup applies the requested model without persisting it", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));

  // Chat-side selections reach the session itself...
  assert.match(startupSource, /requestedModel: effectiveInitialModel/);
  // ...but never pi's global defaultModel/defaultThinkingLevel.
  assert.doesNotMatch(startupSource, /setDefaultModelAndProvider/);
  assert.doesNotMatch(startupSource, /setDefaultThinkingLevel/);
});

test("model switches never rewrite pi's global defaults", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const setModelSource = source.slice(
    source.indexOf('case "set_model"'),
    source.indexOf('case "fork"'),
  );

  assert.match(setModelSource, /await refreshPiWebLlamaModels\(\s*this\.inner\.modelRuntime/);
  assert.match(setModelSource, /await this\.inner\.setModel\(model\);/);
  assert.doesNotMatch(setModelSource, /persist/);
  // The auditable write path lives in the settings API route alone.
  assert.doesNotMatch(source, /setDefaultModelAndProvider/);
  assert.doesNotMatch(source, /setDefaultThinkingLevel/);
});
...
test("normal sessions restore persisted tool selections before loading resources", async () => {
  ...
  assert.match(startupSource, /chatOnly\s*\? withPiWebLlamaProvider\(CHAT_ONLY_RESOURCE_LOADER_OPTIONS\)/);
