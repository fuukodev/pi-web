...
test("returns bounded assistant/tool detail without image base64 payloads", () => {
  ...
  assert.equal(detail.payload.type, "toolCall");
  assert.deepEqual(detail.payload.input, { path: "src/index.ts" });
  assert.deepEqual(detail.timing, { durationMs: 1000, durationSource: "estimated" });
...
test("returns only the selected text block as its payload", () => {
  const detail = buildTrajectoryRecordDetail([
    user("u1", null),
    assistant("a1", "u1"),
  ], "a1", undefined, "text:a1:2");

  assert.deepEqual(detail.payload, { type: "text", text: "I inspected it." });
...
test("resolves a thinking record by id instead of the assistant record", () => {
  ...
  assert.deepEqual(detail.payload, { type: "thinking", thinking: "private reasoning" });
  assert.throws(
