...
test("searches each thinking block independently", () => {
  const entries = [
    user("u1", null, "inspect"),
    assistant("a1", "u1", [
      { type: "thinking", thinking: "needle is in the first block" },
      { type: "thinking", thinking: "this block has no match" },
    ]),
  ];
  const response = buildTrajectorySearch(entries, null, { query: "needle", types: ["thinking"] });
  assert.deepEqual(response.matches.map((match) => match.record.id), ["thinking:a1"]);
  assert.equal(response.matches[0].snippet, "needle is in the first block");
});
