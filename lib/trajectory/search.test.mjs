...
test("matches user, assistant, and thinking content with distinct fields", () => {
  ...
  const assistantMatch = byId.get("entry:a1");
  assert.equal(assistantMatch.field, "text");
  assert.equal(assistantMatch.turnOrdinal, 1);
...
test("filters by record type without leaking thinking into assistant matches", () => {
  ...
  assert.deepEqual(assistantOnly.matches.map((match) => match.record.id), ["entry:a1"]);
...
test("matches case-insensitively and across collapsed whitespace", () => {
  ...
  assert.deepEqual(response.matches.map((match) => match.record.id), ["entry:a1", "entry:u1"]);
...
test("only searches records on the selected active branch", () => {
  ...
  assert.deepEqual(response.matches.map((match) => match.record.id), ["entry:a2"]);
});

test("rejects invalid queries, types, and limits", () => {
  const entries = fixtures();
  const invalid = (options) => (error) => error instanceof TrajectorySearchError && error.code === "invalid_query";
