...
test("accepts the projected turn id as an anchor for search jumps", () => {
  const byTurnId = buildTrajectoryPage(entries, null, { limit: 2, anchorTurnId: "turn:u2" });
  const byStartEntryId = buildTrajectoryPage(entries, null, { limit: 2, anchorTurnId: "u2" });

  assert.deepEqual(byTurnId.turns.map((turn) => turn.id), ["turn:u1", "turn:u2"]);
  assert.deepEqual(
    byTurnId.turns.map((turn) => turn.id),
    byStartEntryId.turns.map((turn) => turn.id),
  );
  assert.equal(byTurnId.hasLater, true);
});
