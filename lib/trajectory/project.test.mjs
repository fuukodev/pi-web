...
test("projects multiple assistant tool calls and pairs each result", () => {
  ...
  assert.deepEqual(records.turns[0].records.map((record) => record.kind), [
    "user", "assistant", "thinking", "tool", "tool",
  ]);
...
test("keeps thinking separate from assistant text records", () => {
  ...
  assert.deepEqual(records.turns[0].records.map((record) => record.kind), ["user", "assistant", "thinking"]);
  const [, textRecord, thinkingRecord] = records.turns[0].records;
  assert.equal(textRecord.preview, undefined);
  assert.equal(thinkingRecord.preview, "Weighing options.\n\nPicking one.");
});
