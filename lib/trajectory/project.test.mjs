...
test("projects multiple assistant tool calls and pairs each result", () => {
  ...
  assert.deepEqual(records.turns[0].records.map((record) => record.kind), [
    "user", "assistant", "thinking", "tool", "tool", "text",
  ]);
  const thinking = records.turns[0].records[2];
  assert.equal(thinking.id, "thinking:a1");
  assert.equal(thinking.parentId, "entry:a1");
  assert.equal(thinking.summary, "thinking");
  assert.match(thinking.preview, /I should inspect both files\./);
  ...
  assert.equal(firstTool.preview, 'read (path="a.ts")');
  assert.equal(firstTool.resultPreview, "result for tc1");
  ...
  assert.equal(secondTool.preview, 'read (path="b.ts")');
  assert.equal(secondTool.error, "result for tc2");
  assert.equal(secondTool.resultEntryId, "r2");
  assert.equal(secondTool.parentId, "entry:a1");
  assert.equal(records.turns[0].records[1].preview, undefined);
  assert.equal(records.turns[0].records[1].durationMs, 5000);
  assert.equal(records.turns[0].records[1].durationSource, "estimated");
  assert.equal(records.turns[0].records[1].agentRunId, "entry:a1");
  assert.equal(records.turns[0].records[1].toolCallCount, 2);
  assert.equal(records.turns[0].records[5].kind, "text");
  assert.equal(records.turns[0].records[5].preview, "I inspected the project.");
  assert.equal(records.turns[0].records[5].agentRunId, "entry:a1");
});

test("keeps thinking separate from assistant text records", () => {
  const records = projectTrajectory([
    user("u1", null, "think about it", t0),
    assistant("a1", "u1", [
      { type: "thinking", thinking: "Weighing options." },
      { type: "thinking", thinking: "Picking one." },
    ], t1),
  ]);

  assert.deepEqual(records.turns[0].records.map((record) => record.kind), ["user", "assistant", "thinking", "thinking"]);
  const [, assistantRecord, firstThinking, secondThinking] = records.turns[0].records;
  assert.equal(assistantRecord.preview, undefined);
  assert.equal(firstThinking.preview, "Weighing options.");
  assert.equal(secondThinking.preview, "Picking one.");
});
...
test("retains orphaned tool results and assistant errors without inventing timing", () => {
  ...
  assert.equal(turn.records[1].stopReason, "error");
  assert.equal(turn.finalStatus, "error");
  const orphan = turn.records[3];
