...
test("preserves an existing API key inside the final credential write", async () => {
  await withAuthFile({
    "llama.cpp": { type: "api_key", key: "current-secret", env: { LLAMA_BASE_URL: "http://old" } },
  }, async (authPath) => {
    await storeProviderCredential(
      "llama.cpp",
      { type: "api_key", env: { LLAMA_BASE_URL: "http://new" } },
      authPath,
      { preserveExistingApiKey: true },
    );

    assert.deepEqual(await readAuthFile(authPath), {
      "llama.cpp": { type: "api_key", key: "current-secret", env: { LLAMA_BASE_URL: "http://new" } },
    });
  });
});
